...
> Status: reproducible snapshot slice implemented; task, agent, environment, PR, release, and MCP orchestration remain target architecture.
...
- normalized manifest and `.gitmodules` SHA-256;
- SHA-256 evidence for every explicitly declared pinned-commit file or canonical Git tree.
...
The snapshot harness is implemented as a narrow Rust workspace and exposes the root `./ws` launcher.
The extra orchestration crates and MCP server in the target layout remain planned.
...
- validates dependency references and returns deterministic dependency order;
